...
-- It was originally created for jekyll4ht
--
...
-- default conversion table is created
local codes = prepare_table(conversion_table)

-- local gsub = unicode.utf8.gsub

-- k is token table, tb conversion table
local strip_accents = function(k,tb)
	local tb =  tb or codes
  local status = false
  for i= 1, #k do 
    local c = k[i]
    status = status or tb[c]
    k[i] = tb[c] or c
  end
  return k, status
end
